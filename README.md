...
| authenticate | http://127.0.0.1:8080/function/authenticate | POST | `{ "username": "foo", "password": string, "totp_code": number }` | `{ "authenticated": true }` | `400` and `{ "authenticated": false }` if username, password or TOTP code are missing, or if any of these are invalid. `401` if password expired. `500` if internal server error |